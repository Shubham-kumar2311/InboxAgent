...
class GmailToolsClass:
            ...
            for email in recent_emails:
                ...
                if thread_id not in seen_threads and thread_id not in threads_with_drafts:
                    ...
                    if self._should_skip_email(email_info):
                        print(f"[gmail] skipped self email: sender='{email_info.get('sender')}'")
                        continue
    ...
    def fetch_recent_emails(self, max_results=50):
        try:
            ...
            print(f"[gmail] fetch_recent_emails: query='{query}' results={len(messages)}")
    ...
    def fetch_draft_replies(self):
        ...
        try:
            ...
            print(f"[gmail] fetch_draft_replies: drafts={len(draft_list)}")
            return [
                {
                    "draft_id": draft["id"],
    ...
    def _get_gmail_service(self):
        creds_path = os.environ.get('CREDENTIALS_PATtippireddy.reddy23bH', 'credentials.json')
        ...
        return build('gmail', 'v1', credentials=creds)  # ← also apply the bug fix from earlier
    ...
    def _log_email_read(self, email_info, body_limit=500):
        body = (email_info.get("body") or "").strip()
        preview = body[:body_limit]
        ...
        print(
            ...
            f"threadId='{email_info.get('threadId')}', "
